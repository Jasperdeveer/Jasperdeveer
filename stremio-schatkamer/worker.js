...
function parseSearchResults(html, skip) {
  ...
  // Stap 1: probeer JSON-data in de HTML te vinden (Next.js __NEXT_DATA__ o.i.d.)
  ...
  // Stap 2: zoek alle interne links die eruitzien als content-pagina's
  if (items.length === 0) {
    ...
    while ((m = linkRe.exec(html)) !== null) {
      ...
      // Zoek de directe omgeving van de link (±500 tekens) voor titel en afbeelding
...
export default {
  async fetch(request) {
    ...
    // Catalog: /catalog/movie/bg-browse.json?extra=...
    ...
    if (metaM) {
      ...
      try {
        ...
        // Stuur altijd iets terug zodat Stremio de item-pagina toont
        const fallback = {
          id: bgId,
          type: 'movie',
